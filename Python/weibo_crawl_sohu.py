...
def search_for_new_statuses():
    previous_real_count = session.query(Status).filter(Status.info_source_id==SOHU_WEIBO_INFO_SOURCE_ID).count()
    ...
    current_real_count = session.query(Status).filter(Status.info_source_id==SOHU_WEIBO_INFO_SOURCE_ID).count()
